...
int main(int argc, char *argv[]) {
  ...
  boost::posix_time::ptime timeout =
      ...
      boost::posix_time::seconds(kMaxParamTimeout);
  ...
  std::pair<ipcb::IPCParam *, bip::managed_shared_memory::size_type>
      param_pair = shared_mem->find<ipcb::IPCParam>(sem_param_name.c_str());
  ...
  try {
    ...
    ipcb::IPCResult *result =
        shared_mem->find_or_construct<ipcb::IPCResult>(sem_result_name.c_str())(
            string_allocator, bytes_allocator, time_allocator);
    if (result == nullptr) {
