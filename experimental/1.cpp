...
int main() {
  ...
  while (
    (p_cert_context = CertEnumCertificatesInStore(h_store, p_cert_context))) {
    std::cout << delimiter;
    // encoding
    ...
    // issuer
    {
      ...
      DWORD dwSize = CertNameToStrA(X509_ASN_ENCODING, &issuerNameBlob,
                                    CERT_X500_NAME_STR, nullptr, 0);
      std::string issuerString(dwSize, '\0');
      CertNameToStrA(X509_ASN_ENCODING, &issuerNameBlob, CERT_X500_NAME_STR,
                     &issuerString[0], dwSize);
      std::cout << issuerString << "\n";
    }
    // name
    {
      CERT_NAME_BLOB& subject_blob = p_cert_context->pCertInfo->Subject;
      DWORD dwSize = CertNameToStrA(X509_ASN_ENCODING, &subject_blob,
                                    CERT_X500_NAME_STR, nullptr, 0);
      std::cout << "Name = ";
      std::string cert_name(dwSize, '\0');
      CertNameToStrA(X509_ASN_ENCODING, &subject_blob, CERT_X500_NAME_STR,
                     &cert_name[0], dwSize);
      std::cout << cert_name << "\n";
    }
  }
  ...
  */
  ...
  PCCERT_CONTEXT p_cert_ctx =
    CertFindCertificateInStore(h_store, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING,
  ...
  */
  ...
  // get sing size
  DWORD sign_size = 0;
  res = CryptMsgGetParam(handler_message, CMSG_CONTENT_PARAM, 0,
                         0 /* just size*/, &sign_size);
  ...
  handler_message = CryptMsgOpenToDecode(
    X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0, 0, 0, 0, 0);
  if (handler_message == 0) {
    std::cout << "Open to decode ... FAIL\n";
  }
  res = CryptMsgUpdate(handler_message, message_data.data(),
  ...
  res = CryptMsgGetParam(handler_message, CMSG_ENCODED_MESSAGE, 0,
                         message_data.data(), &sign_size);
