...
TEST_CASE("UnparseARM") {
  ...
  for (uint i = 0; i < pdf.GetSignaturesCount(); ++i) {
    AsnObj obj(pdf.getRawSignature(i).data(), pdf.getRawSignature(i).size(),
               std::make_shared<ResolvedSymbols>());
    auto unparsed = obj.Unparse();
    std::cout << "Initial size " << pdf.getRawSignature(i).size() << "\n";
    std::cout << "Unparsed size " << unparsed.size() << "\n";
    if (unparsed.size() < pdf.getRawSignature(i).size()) {
      unparsed.resize(pdf.getRawSignature(i).size(), 0x00);
    }
    REQUIRE(unparsed.size() == pdf.getRawSignature(i).size());
    REQUIRE(unparsed == pdf.getRawSignature(i));
  }
}
    ...
    for (uint i = 0; i < pdf.GetSignaturesCount(); ++i) {
      ...
      for (uint signer_index = 0; signer_index < signers.value();
        ...
        std::cout << "Type:"
                  ...
                  << "\n";
        REQUIRE(msg->CheckDataHash(pdf.getRawData(i), signer_index));
  ...
  SECTION("05_acrob_BES") {
    ...
    for (uint i = 0; i < pdf.GetSignaturesCount(); ++i) {
      std::cout << "\nTest signature " << i + 1 << " of "
                << pdf.GetSignaturesCount() << "\n";
      REQUIRE_NOTHROW(msg = csp.OpenDetached(pdf.getRawSignature(i)));
      std::ofstream outp_file("05_acrob_BES.dat" + std::to_string(i),
                              std::ios_base::binary);
  ...
  SECTION("07_acrob_CADEST") {
    ...
    for (uint i = 0; i < pdf.GetSignaturesCount(); ++i) {
      std::cout << "\nTest signature " << i + 1 << " of "
                << pdf.GetSignaturesCount() << "\n";
      REQUIRE_NOTHROW(msg = csp.OpenDetached(pdf.getRawSignature(i)));
      std::ofstream outp_file("07_acrob_CADEST.dat" + std::to_string(i),
                              std::ios_base::binary);
      for (const auto ch : pdf.getRawSignature(i)) {
        outp_file << ch;
      }
