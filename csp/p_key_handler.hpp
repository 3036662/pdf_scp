...
public:
   PKeyHandler(PCCERT_CONTEXT cert_contex,PtrSymbolResolver symbols):symbols_(std::move(symbols)){
    ...
    ResCheck(symbols_->dl_CryptAcquireCertificatePrivateKey(
      ...
      &caller_must_free_),"CryptAcquireCertificatePrivateKey",symbols);
    if (h_csp_==0){
        throw std::runtime_error("[PKeyHandler] failed to get a private key for the certificate");
    }
   }
